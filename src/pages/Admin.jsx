...
// Empty card template — matches our schema
const blankCard = () => ({
  id: "",
  slug: "",
  name: "",
  ...
  types: [],
  domains: [],
  tags: "",                // string in form, split on save
  rarity: "Common",
  energyCost: "",
  powerCost: 0,
  alternateCost: false,
  ...
  cardText: "",
  flavorText: "",
  keywords: "",            // string in form, split on save
  imageArtist: "",
  imageUrl: "",
  legalStandard: true,
  legalCasual: true,
...
export default function Admin() {
  ...
  // Try to authenticate by hitting an admin route
  const tryAuth = async () => {
    try {
      const res = await fetch(`${API_URL}/admin/cards`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "X-Admin-Password": password },
        body: JSON.stringify({}),    // empty body — will fail validation but auth will pass
      })
      if (res.status === 401) {
  ...
  // Auto-derive slug and id from name + set + number
  ...
  const updateNumber = (number) => {
    setCard(c => ({ ...c, number, id: `${c.set}-${number}` }))
  }
  ...
  // Toggle a value in an array field (types or domains)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Convert form fields to schema fields
      const payload = {
        ...card,
        tags:        card.tags.split(",").map(t => t.trim()).filter(Boolean),
        keywords:    card.keywords.split(",").map(k => k.trim()).filter(Boolean),
  ...
  return (
    <div className="max-w-5xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold" style={{ color: "var(--text-primary)" }}>Add Card</h1>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="number" value={card.energyCost} onChange={e => setCard(c => ({ ...c, energyCost: e.target.value }))}
            placeholder="Leave empty for Legends"
            className="rounded-lg px-3 py-2 focus:outline-none" style={inputStyle} />
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="number" value={card.might} onChange={e => setCard(c => ({ ...c, might: e.target.value }))}
            placeholder="Only for Champion/Unit/Token"
            className="rounded-lg px-3 py-2 focus:outline-none" style={inputStyle} />
        </div>
      ...
      <div className="mb-6 flex flex-col gap-1">
        ...
        {card.imageUrl && (
          <p className="text-xs mt-1" style={{ color: "var(--text-dim)" }}>
            Current: {card.imageUrl.split("/").pop()}
          </p>
        )}
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={() => { setCard(blankCard()); setImageFile(null); setStatus("") }}
          className="px-6 py-3 rounded-lg" style={{ color: "var(--text-secondary)", border: "1px solid var(--border)" }}>
          Clear Form
        </button>
